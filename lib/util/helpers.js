...
export function handleApiError(error, tag, operation) {
  ...
  if (error.response?.data) {
    ...
    if (typeof error.response.data.error === 'string') {
      ...
      err.status = error.response.status || error.code || 400
      throw err
      ...
      err.status = code || error.response.status || 400
      err.code = code
      throw err
    } else {
      const err = new Error(`API Error: ${JSON.stringify(error.response.data)}`)
